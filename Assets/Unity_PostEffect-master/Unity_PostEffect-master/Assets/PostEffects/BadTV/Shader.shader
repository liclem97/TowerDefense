Shader "Custom/Shader" {
	...
	SubShader {
		...
		Pass {
			...
			// 定義済みヘルパー関数インクルード 정의된 도우미 함수 인클루드
			#include "UnityCG.cginc"

			// 定義されている頂点シェーダー関数 정의된 정점 셰이더 함수
			#pragma vertex vert_img
			// フラグメントシェーダー関数 프래그먼트 셰이더 함수
			...
			float snoise(fixed2 v) {
				...
				x12.xy -= i1;
				i = mod289(i); // Avoid truncation effects in permutation", 순열에서 절단 효과를 피하세요,"
			...
			fixed4 frag (v2f_img i): COLOR
			{
				...
				//return c;
				  ...
				  float t = _Time.y;
                  float scrollAmount = rollSpeed * exp(-t * 0.5); // exp 곱해서 부드럽게 감쇠
                  float scrollUV = fract(i.uv.y - t * scrollAmount);

                      // 텍스처 좌표 적용
                  fixed4 c = tex2D(_MainTex, fixed2(fract(i.uv.x + offset), scrollUV));
                   return c;
			}
			ENDCG
		}
